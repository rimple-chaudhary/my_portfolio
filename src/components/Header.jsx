    ...
    <header className="fixed top-0 left-0 w-full z-50 bg-gray-900/95 backdrop-blur-md md:bg-transparent md:backdrop-blur-none">
      <div className="md:fixed md:top-4 md:left-1/2 md:transform md:-translate-x-1/2 w-full md:w-auto">
        <div className="p-[2px] md:rounded-full bg-gradient-to-r from-emerald-400 via-cyan-500 to-indigo-500 animate-gradient-x">
          <nav className="bg-gray-900/90 backdrop-blur-md md:rounded-full px-4 md:px-6 py-2.5" aria-label="Primary">
            {/* Mobile Menu Button */}
            ...
            {/* Navigation Links */}
            <div id="primary-navigation" className={`${isMenuOpen ? 'block' : 'hidden'} md:block`}>
                ...
                {navLinks.map(({ id, icon: Icon, text, path }) => (
                  <Link
                    ...
                    className={`px-3 py-2 md:py-1.5 rounded-lg md:rounded-full text-sm font-medium
                      ...
                      hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400
                      ${
                        activeLink === id
